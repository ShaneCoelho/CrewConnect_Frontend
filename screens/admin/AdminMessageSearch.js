...
const AdminMessageSearch = ({navigation,navigation:{goBack}}) => {
  ...
  const handleSelectedEmployee = (id,employeeId,name,designation,phone,email,address,Avatar) => {
    ...
    console.log(phone)
    navigation.navigate('AdminSendMessage', { id, employeeId, name, designation, phone, email, address,Avatar});
  };

  const renderItem = ({ item }) => (
    <TouchableOpacity style={styles.contactItem} onPress={()=>handleSelectedEmployee(item._id,item.employeeId,item.name,item.designation,item.phone,item.email,item.address,item.Avatar)}>
