...
import { View, Text, StyleSheet, FlatList, ScrollView, Dimensions, TouchableOpacity, BackHandler, Alert } from 'react-native';
...
const AdminDashboard = ({ navigation }) => {
  ...
  const fetchLeaveRequests = async () => {
    ...
    try {
      ...
      const response = await fetch('https://crewconnect.onrender.com/displayleave', {
        method: 'POST',
      });
  ...
  const fetchWfhRequests = async () => {
    ...
    try {
      ...
      const response = await fetch('https://crewconnect.onrender.com/displaywfh', {
        method: 'POST',
      });
      const responseData = await response.text();
      //console.log(responseData); // Log the response data
  ...
  const fetchUpcomingMeeting = async () => {
    const response = await fetch('https://crewconnect.onrender.com/fetchmeetinginfo', {
      method: 'POST',
      headers: {
  ...
  const handleWfhPress = (id, name, fromdate, todate, reason) => {
    // Navigate to another screen and pass employeeId as a prop
    navigation.navigate('AdminWfh', { id, name, fromdate, todate, reason });
  };
  ...
  const renderWfhRequest = ({ item }) => (
    <TouchableOpacity onPress={() => handleWfhPress(item.id, item.name, item.fromdate, item.todate, item.reason)}>
      <View style={styles.requestItem}>
        <Text style={styles.employeeName}>{item.name}</Text>
        <View style={styles.dateContainer}>
          <Text style={styles.dateText}>{item.fromdate}</Text>
